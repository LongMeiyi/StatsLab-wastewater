...
get_vector_constant_waiting_time_distr <- function(shape_incubation,
                                                   ...
                                                   length_out = 200,
                                                   n_random_samples = 1E6){ #这个parameter没有用 {
...
get_bootstrap_replicate <- function(original_time_series, block_size = 10, days_incl = 21) { # to sample one timse series based on LOESS and block bootstrap
...
block_boot_overlap_func <- function(ts, block_size = 10){ # to sample residuel/ difference of LOESS smoothing, ts stands for time series
...
do_deconvolution <- function(
  incidence_data,
  days_further_in_the_past = 30, # this is the dimension of the deconvolution function, i.e. how many days after the infection
  verbose = FALSE,
  delay_distribution_matrix,
...
get_infection_incidence_by_deconvolution <- function(
  ...
  data_type_subset <- unique(data_subset$data_type)[1] # what is this for? Seems like only for printing out something?
  ...
  is_empirical = (nrow(empirical_delays) > 0) # boolean variable to determine the mode of the function
  ...
  if( is_onset_data ) {
  ...
  } else { # ! our case: if not on-set data
    if(is_empirical) {
      delay_distribution_matrix_onset_to_report <- get_matrix_empirical_waiting_time_distr(
        empirical_delays,
    ...
    } else { # ! our case: fixed onset delay distribution
  ...
  for (bootstrap_replicate_i in 0:n_bootstrap) {
    ...
    } else { # smooth_incidence == F, our case
      smoothed_incidence_data <- time_series  %>%
        complete(date = seq.Date(min(date), max(date), by = "days"), fill = list(value = 0))
    }
    ...
    results <- c(results, list(deconvolved_infections)) # it returns deconvolved values for each bootstrap
  }
