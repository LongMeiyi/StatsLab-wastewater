...
# Import Code from Deconvolution Code and Functions
...
# We filter after Sept 1 because the data prior is often below LOQ (Limit of quantification, terminology in epidemiology concerning viral RNA measurements)
...
config_df = expand.grid("region" = c('ZH'),  # To create all combinations of parameter settings
                        'incidence_var' = c('norm_n1', 'norm_n2'),
                        'FirstGamma' = 'incubation',
                        'SecondGamma' = 'benefield' ) # benefield is the distribution proposed by other clinical paper.


deconv_ww_data <- data.frame()
Re_ww <- data.frame()

for(row_i in 1:nrow(config_df)){ # Iterate over different parameter settings for deconvolveIncidence()
  new_deconv_data = deconvolveIncidence(ww_data %>% filter(region == config_df[row_i, 'region']), 
                                        ...
                                        getCountParams(as.character(config_df[row_i, 'FirstGamma'])), # Two delay distributions for deconvolution instead of one
