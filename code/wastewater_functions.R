...
addUselessColumns <- function(df, inc_var = 'n1'){ 
  if (!'region' %in% colnames(df)){ ## if not specify region, then region is "CHE"
    df <- df %>%
      mutate(region = 'CHE')
  }
  
  observation_df <- df %>% # add new columns 
  ...
  observation_df <- observation_df %>% # delete missing values
    filter(!is.na(value))
  
  return(observation_df)
}

# wrapper around the actual deconvolution function, use get_infection_incidence_by_deconvolution in shiny package
deconvolveIncidence <- function(df, incidence_var = 'n1',
                                IncubationParams, OnsetToCountParams,
                                smooth_param = FALSE, n_boot = 50){
  infection_df <- addUselessColumns(df, inc_var = incidence_var) # Add one column to the original data set for output.
  
  # list of two discretized pdf, one for starting from infection (effects of two gamma distribution) , one for starting from onset
  constant_delay_distributions <- list("Simulated" = get_vector_constant_waiting_time_distr( # this function is to return discretized pdf of the gamma distribution
  ...
  estimatedInfections <- get_infection_incidence_by_deconvolution( # this function is also in shiny
    infection_df,
    is_local_cases = T,
